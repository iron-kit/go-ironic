...
func getHeader(key string, req *go_api.Request) string {
...
func (b *DefaultBinder) Bind(req *go_api.Request, in interface{}) error {
	if len(req.Body) == 0 {
		if req.Method == GET || req.Method == DELETE {
			if err := b.bindData(in, b.parseParams(req.Get), "query"); err != nil {
				return err
			}
			return nil
		}
	...
	contentType := getHeader(HeaderContentType, req)
...
func (b *DefaultBinder) parseParams(p map[string]*go_api.Pair) map[string][]string {
